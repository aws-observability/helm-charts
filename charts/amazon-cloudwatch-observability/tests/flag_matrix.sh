...
#   (b) asserting that the two invalid combinations (#3, #4) fail helm template
...
#   $3  otelContainerInsights.logs     (true|false)
#   $4  containerLogs.enabled          (true|false)
#   $5  expected outcome: "ok" (renders successfully) or "fail" (helm template
#       must fail with the logs-requires-enabled error)
...
run_case() {
    ...
    output=$(helm template "$CHART_DIR" \
        ...
        --set "otelContainerInsights.logs=$logs" \
    ...
    if [[ "$expected" == "fail" ]]; then
        ...
        if ! grep -q "otelContainerInsights.logs=true requires otelContainerInsights.enabled=true" <<< "$output"; then
            echo -e "  ${R}FAIL${N}: helm failed, but not with the expected validation message"
            echo "  actual: $output" | head -3
            fail_count=$((fail_count + 1))
            return
        fi
        echo -e "  ${G}PASS${N}: helm template failed with expected validation message"
...
# State #3: invalid — logs=true without enabled.
run_case 3 false true false "fail" \
    "INVALID — logs=true requires enabled=true"

# State #4: invalid — same as #3, FluentBit would-be on.
run_case 4 false true true "fail" \
    "INVALID — logs=true requires enabled=true (FluentBit flag irrelevant)"
