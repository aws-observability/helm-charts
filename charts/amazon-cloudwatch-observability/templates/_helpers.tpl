...
the configured targetAgent, and otelContainerInsights.prometheusScrape.enabled is true
(the single opt-out switch). Individual monitor types are NOT considered here — use
cloudwatch-agent.serviceMonitorEnabled / cloudwatch-agent.podMonitorEnabled for those.
Accepts a dict with "agentName" (string) and "context" (root context $).
*/}}
