...
- **Cluster-scraper ClusterRole** (`otel-container-insights-cluster-scraper-clusterrole.yaml`) — RBAC for the cluster-scraper agent. Contains ServiceAccount and ClusterRole/ClusterRoleBinding (for pods/nodes/endpoints/services access). Lease RBAC is on the shared agent role. Gated by `otelContainerInsights.enabled`.
