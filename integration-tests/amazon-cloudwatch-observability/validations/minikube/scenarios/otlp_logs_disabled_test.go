...
// otelContainerInsights.logs=false, containerLogs.enabled=false.
//
...
// the generated config. The shared names are gated by otelContainerInsights.logs.
func assertLogPipelineAbsent(t *testing.T, otelConfig string) {
	...
	for _, fragment := range logOnlyFragments {
		assert.False(t, strings.Contains(otelConfig, fragment),
			"otelConfig must not contain %q when otelContainerInsights.logs=false", fragment)
	}
}

// assertNoLogMounts verifies the CWA DaemonSet does not carry the /var/log or
// journald host mounts, which are only needed when otelContainerInsights.logs=true.
